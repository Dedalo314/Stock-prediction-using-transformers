...
filehandler = logging.FileHandler(filename='logs/transformer_orpheus4classifier.log', mode="w+")
...
try:
    ...
    train_data = EzStockBinaryClassDataset(close_data=df["close"].tolist(), num_stock_values_in=NUM_STOCK_VALUES_IN,
                                num_stock_to_avg=NUM_STOCK_TO_AVG, for_training=True)
    validation_data = EzStockBinaryClassDataset(close_data=df["close"].tolist(), num_stock_values_in=NUM_STOCK_VALUES_IN,
    ...
    orpheus = Orpheus4Classifier(d_model=D_MODEL, nhead=N_HEAD, max_seq_length=NUM_STOCK_VALUES_IN,
